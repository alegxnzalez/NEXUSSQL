...
window.RATE_OFFERING = {
  "tables": {
    "RATE_OFFERING": {
      "description": "The contract between the carrier and the consignee (or service provider.  One record per contract.",
      "columns": {
        "RATE_OFFERING_GID": {
          "type": "VARCHAR2(101)",
          "nullable": false,
        ...
        "CIRCUITY_ALLOWANCE_PERCENT": {
          "type": "NUMBER",
          "nullable": true,
          "info": "Circuity Allowance Percent indicates the allowed amount of out of route distance (as a percent of the direct source to final destination distance) that is free/ before Circuitous Distance Cost are charged.  Circuity Allowance Percent = (t/d-1)*100 where: t is the allowed total distance traveled between the first and last stop of the shipment. d is the shortest distance between the first and last stop of the shipment. If the allowed total distance is 1500 miles and the distance between the first and last stop is 1300 miles, Circuity Allowance Percent = (1500/1300-1)*100 = (1.15-1)*100 = 15%. The lower the percentage, the more in line the route must be."
},
        ...
        "ALLOW_UNCOSTED_LINE_ITEMS": {
          ...
          "info": "Allow Uncosted Line Items (Y/N) is used when there are one or more ratable objects on the shipment which are not charged or considered for rating when rating a shipment using rate record. Marked: Some basis options within a shipment pass through the rating engine without a charge if other basis options qualify. For example, if a shipment contains four line items, and a rate is found that contains charges for 3 of those items, that rate qualifies for the shipment and the fourth line item ships for free."
},
        ...
        "TRACK_CAPACITY_USAGE": {
          ...
          "info": "The Track Capacity Usage field when set to Y, and the Capacity Group field if provided, causes the system to track capacity usage during the planning process. If the Track Capacity Usage is N or the Capacity Group field is blank, the system will not track capacity usage during the planning process. Values (Y/N)."
},
        "ROUNDING_TYPE": {
          "type": "VARCHAR2(1)",
          "nullable": true,
        ...
        "DOMAIN_PROFILE_GID": {
          "type": "VARCHAR2(101)",
          "nullable": true,
          "info": "Providing a domain profile allows for the rate offering to be constrained to the domains in the domain profile and serves as a constraint on the rate offering. If none of the rate offerings have a customer domain profile constraint, then all rate offerings are returned. If some of the rate offerings have customer domain profiles, the rate offerings are grouped by service provider. For each service provider, only the offerings whose domain profile constraint is satisfied are returned. So, given two rate offerings, one with a constraint, and one without a constraint: if the order release domain list is compatible with the constraint, only the rate offering with the constraint is returned, otherwise, only the offering without the constraint is returned."
},
        ...
        "SERVPROV_PROFILE_GID": {
          ...
          "info": "A unique identifier to define the group profile for multiple service providers."
},
        "RATE_SERVICE_PROFILE_GID": {
          "type": "VARCHAR2(101)",
          "nullable": true,
          "foreign_key": "RATE_SERVICE_PROFILE",
          "info": "A unique identifier to define the group profile for multiple rate services."
},
        "MODE_PROFILE_GID": {
          "type": "VARCHAR2(101)",
          "nullable": true,
          "foreign_key": "MODE_PROFILE",
          "info": "A unique identifier to define the group profile for multiple modes."
}
      },
      "fk_tables": [
        "ACTIVITY_TIME_DEF",
